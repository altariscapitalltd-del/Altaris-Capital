          ...
          <div style={{ display:'flex', gap:6 }}>
            {(['weekly', 'monthly'] as const).map(p => (
              <button key={p} onClick={() => setLbPeriod(p)}
                style={{ flex:1, padding:'9px', borderRadius:99, border:'none', background:lbPeriod===p?'#F2BA0E':'#111', color:lbPeriod===p?'#000':'#555', fontWeight:lbPeriod===p?700:500, fontSize:12, cursor:'pointer', fontFamily:'inherit', border:`1px solid ${lbPeriod===p?'#F2BA0E':'rgba(255,255,255,0.06)'}` }}>
