...
function ClockPendingIcon() { return <svg width="72" height="72" viewBox="0 0 72 72" fill="none"><circle cx="36" cy="36" r="28" stroke="#F2BA0E" strokeWidth="2" fill="rgba(242,186,14,0.08)"/><path d="M36 22v14l9 5" stroke="#F2BA0E" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg> }
...
function StepDot({ idx, current }: { idx: number; current: number }) {
  ...
  return <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}><div style={{ width:32, height:32, borderRadius:'50%', background: completed ? '#0ECB81' : active ? '#F2BA0E' : '#1A1A1A', border:`2px solid ${completed ? '#0ECB81' : active ? '#F2BA0E' : 'rgba(255,255,255,0.08)'}`, display:'flex', alignItems:'center', justifyContent:'center' }}>{completed ? '✓' : <span style={{ fontSize:12, fontWeight:800, color: active ? '#000' : '#555' }}>{idx + 1}</span>}</div><span style={{ fontSize:9, color: active ? '#F2BA0E' : completed ? '#0ECB81' : '#444', fontWeight:600, textAlign:'center', maxWidth:60, lineHeight:1.2 }}>{STEPS[idx].label}</span></div>
}
...
function InfoBox({ title, desc }: any) { return <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:16 }}><div style={{ display:'flex', gap:10 }}><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#F2BA0E" strokeWidth="2" style={{ flexShrink:0, marginTop:2 }}><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><div><div style={{ fontWeight:700, fontSize:13, marginBottom:3 }}>{title}</div><div style={{ color:'var(--text-muted)', fontSize:12, lineHeight:1.5 }}>{desc}</div></div></div></div> }
...
function CenterSpinner() { return <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'60vh' }}><div style={{ width:32, height:32, border:'3px solid rgba(242,186,14,0.2)', borderTopColor:'#F2BA0E', borderRadius:'50%', animation:'spin .8s linear infinite' }} /></div> }
