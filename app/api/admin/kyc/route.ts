...
export async function PATCH(req: NextRequest) {
  ...
  if (action === 'approve') {
    ...
    await notifyUser(prisma, userId, 'KYC Approved', 'Your identity has been verified. You can now make withdrawals.', '/kyc')
  } else {
    await prisma.$transaction([
      prisma.kycSubmission.delete({ where: { userId } }),
      prisma.user.update({ where: { id: userId }, data: { kycStatus: 'NOT_SUBMITTED' } }),
    ])
    await notifyUser(prisma, userId, 'KYC Rejected', `KYC rejected: ${reason || 'Please resubmit with a clearer document.'}`, '/kyc')
  }
