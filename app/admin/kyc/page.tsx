...
export default function AdminKYCPage() {
  ...
  function load() {
    fetch('/api/admin/kyc').then(r => r.json()).then(d => setSubmissions(d.submissions || []))
  }
  useEffect(load, [])
  ...
  async function decide(userId: string, action: 'approve' | 'reject') {
    ...
    if (res.ok) {
      setMsg({ type: 'success', text: `KYC ${action === 'approve' ? 'approved' : 'rejected'} — user notified by email.` })
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      {msg && (
        <div style={{ background: msg.type === 'success' ? 'rgba(14,203,129,0.08)' : 'rgba(246,70,93,0.08)', border: `1px solid ${msg.type === 'success' ? 'rgba(14,203,129,0.2)' : 'rgba(246,70,93,0.2)'}`, borderRadius: 10, padding: '11px 16px', marginBottom: 16, fontSize: 13, color: msg.type === 'success' ? '#0ECB81' : '#F6465D' }}>
          {msg.text}
        </div>
      )}
          ...
          <div key={s.id} style={{ background: '#111', border: `1px solid ${s.status === 'PENDING_REVIEW' ? 'rgba(242,186,14,0.15)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 14, padding: 18, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 180 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {[
                  { l: 'Full Name', v: s.fullName },
                  { l: 'DOB', v: s.dateOfBirth },
                  { l: 'Country', v: s.address },
                  { l: 'Doc Type', v: s.documentType?.replace('_', ' ') },
                  { l: 'Submitted', v: new Date(s.submittedAt).toLocaleDateString() },
                ].map(({ l, v }) => (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
              <span style={{ padding: '4px 11px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: statusBg(s.status), color: statusColor(s.status) }}>
                {s.status.replace('_', ' ')}
              </span>
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, backdropFilter: 'blur(8px)', padding: 20 }}
          ...
          <div style={{ background: '#131313', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 17, fontWeight: 800, marginBottom: 4 }}>KYC Review</h3>
                <div style={{ color: '#555', fontSize: 13 }}>{selected.user?.name} · {selected.user?.email}</div>
              </div>
              <button onClick={() => setSelected(null)} style={{ border: 'none', background: 'transparent', color: '#555', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>
            ...
            <div style={{ background: '#1A1A1A', borderRadius: 12, padding: 16, marginBottom: 18 }}>
              {[
                ...
                { l: 'Country / Address', v: selected.address },
                { l: 'Document Type', v: selected.documentType?.replace('_', ' ') || '—' },
                { l: 'Document Number', v: selected.documentNumber || '—' },
                { l: 'Submitted', v: new Date(selected.submittedAt).toLocaleString() },
              ].map(({ l, v }) => (
            ...
            {selected.documentPath && (
              <div style={{ marginBottom: 18 }}>
                ...
                <a href={`/api/admin/kyc/file/${encodeURIComponent(selected.documentPath)}`} target="_blank" rel="noreferrer" style={{ background: '#1A1A1A', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#888', display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            ...
            {selected.selfieFile && (
              <div style={{ marginBottom: 18 }}>
                <div style={{ color: '#444', fontSize: 11, fontWeight: 600, marginBottom: 8, letterSpacing: '0.06em' }}>UPLOADED SELFIE</div>
                <a href={`/api/admin/kyc/file/${encodeURIComponent(selected.selfieFile)}`} target="_blank" rel="noreferrer" style={{ background: '#1A1A1A', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#888', display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            ...
            <label style={{ display: 'block', color: '#444', fontSize: 11, fontWeight: 600, marginBottom: 7, letterSpacing: '0.06em' }}>REVIEW NOTE (sent to user)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)}
              style={{ width: '100%', background: '#1A1A1A', color: '#fff', padding: 12, borderRadius: 9, border: '1px solid rgba(255,255,255,0.07)', fontSize: 13, fontFamily: 'inherit', outline: 'none', resize: 'none', minHeight: 70, boxSizing: 'border-box', marginBottom: 16 }}
              placeholder="Add a note (required for rejection)…" />

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              <button onClick={() => setSelected(null)} style={{ padding: 11, borderRadius: 9, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: '#555', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
              <button onClick={() => decide(selected.userId, 'reject')} disabled={loading}
                style={{ padding: 11, borderRadius: 9, border: '1px solid rgba(246,70,93,0.2)', background: 'rgba(246,70,93,0.08)', color: '#F6465D', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', opacity: loading ? 0.6 : 1 }}>
                {loading ? '…' : 'Reject'}
              </button>
