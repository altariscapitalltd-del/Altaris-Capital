...
export default function AdminKYCPage() {
  ...
  function load() {
    fetch('/api/admin/kyc?status=ALL').then((r) => r.json()).then((d) => setSubmissions(d.submissions || []))
  }
  useEffect(load, [])
  ...
  async function decide(userId: string, action: 'approve' | 'reject') {
    ...
    if (res.ok) {
      setMsg({ type: 'success', text: `KYC ${action}d` })
      setSelected(null)
      setNote('')
      load()
    } else {
    ...
    <div style={{ padding: 28 }}>
          ...
          <div key={s.id} style={{ background: '#111', border: `1px solid ${s.status === 'PENDING_REVIEW' ? 'rgba(242,186,14,0.15)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 14, padding: 18, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 180 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {[{ l: 'Address', v: s.address }, { l: 'DOB', v: s.dateOfBirth }, { l: 'Submitted', v: new Date(s.submittedAt).toLocaleDateString() }].map(({ l, v }) => (
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, backdropFilter: 'blur(8px)', padding: 20 }}
          ...
          <div style={{ background: '#131313', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 28, width: '100%', maxWidth: 520 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              ...
              <button onClick={() => decide(selected.userId, 'reject')} disabled={loading} style={{ padding: '11px', borderRadius: 9, border: '1px solid rgba(246,70,93,0.2)', background: 'rgba(246,70,93,0.08)', color: '#F6465D', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Reject</button>
              <button onClick={() => decide(selected.userId, 'approve')} disabled={loading} style={{ padding: '11px', borderRadius: 9, border: 'none', background: '#F2BA0E', color: '#000', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Approve</button>
            </div>
          </div>
        </div>
      )}
